...
const ROW_KEYS = [
  ...
  "priceLevel","origin","customsNo","availability","specUrl","offerStart",
  "offerEnd","customerId",
];
...
export function validate(rows, params) {
  ...
  // Step 2: Mandatory fields — uses row 0 (=VBA row 2) as the template of which columns are required.
  const templateRow = rows[0];
  const mandatoryKeys = ROW_KEYS.filter((k) => {
    if (EXCLUDED_MANDATORY_KEYS.has(k)) return false;
    return !isBlank(templateRow[k]) && !isNA(templateRow[k]);
  });
